...
TMP_OUTPUT="$(mktemp -t binance-sync.XXXXXX)"
...
build_summary() {
    local lines
    lines="$(grep -E "Binance sync:|completed" "$TMP_OUTPUT" 2>/dev/null || true)"
    if [ -z "$lines" ]; then
...
if [ "$EXIT_CODE" -eq 0 ]; then
    if [ "${DISCORD_NOTIFY_ON_SUCCESS}" = "1" ]; then
        if [ -n "${SUMMARY:-}" ]; then
            ...
            send_discord "Binance sync completed"
        fi
    fi
else
    if [ "${DISCORD_NOTIFY_ON_FAILURE}" = "1" ]; then
        if [ -n "${SUMMARY:-}" ]; then
            send_discord "Binance sync FAILED\n${SUMMARY}"
        else
            send_discord "Binance sync FAILED"
        fi
    fi
